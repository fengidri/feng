...
class Routes( object ):
    """
        记录到某个node 的经历的root 与 dir, 最后一个是目标
    """
    def __init__( self ):
        self.routes = None
        self.index = -1 
...
def open_to_file( path ):
    ...
    try:
        ...
        return 
    flog.loginfo( "routes:%s" % str(routes.routes) )
...
def paths_exp_open( ):
    ...
    print type( line )
    #flog.loginfo( u"paths_exp_open %s" % line)
    try:
...
"""
def get_files( ):
    def moduel_name( path, deep ):
        return '/'.join(path.split( '/' )[ -1*(deep + 1) : ])
    all_files = [  ]
    for node in nodes:
        if node.type != "file" or node.deep == 0:
            continue
        all_files.append( ( None, moduel_name(node.path, node.deep)  , node.path , None) )
    return all_files


def switch_file_quick( ):


    gatekeeper.gatekeeper.request( "/gui/switch_file/files" )
    res = gatekeeper.gatekeeper.response( )

    if res.status() == 200:
        file_path = res.get_data( ) [ 2 ]
        try:
            vim.command( "update" )
